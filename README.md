...
モデルは、認証付きの `GET /v1/models` から利用可能な一覧を取得し、音声・埋め込みモデルを除外して自動選択します。一覧の配列順には依存せず、軽量なチャットモデルを優先します。特定モデルへ固定したい場合だけ、サーバー起動前に `SAKURA_AI_MODEL` 環境変数を設定してください。モデル一覧を取得できずキャッシュもない場合は、古い固定モデルを推測で送信せず、再登録や再試行が必要だと分かるエラーを返します。
