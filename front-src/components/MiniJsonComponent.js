...
exports.MiniJsonComponent = void 0;
class MiniJsonComponent {
    constructor() {
        this.latestLegendEntries = [];
        this.input = document.getElementById('mini-json-input');
        this.prettyOutput = document.getElementById('mini-json-output');
        this.minifiedOutput = document.getElementById('mini-json-minified');
        this.legend = document.getElementById('mini-json-legend');
        this.legendCopyButton = document.getElementById('mini-json-copy-legend');
        this.summary = document.getElementById('mini-json-summary');
        this.errorBox = document.getElementById('mini-json-error');
        this.convertButton = document.getElementById('mini-json-run');
        this.copyButton = document.getElementById('mini-json-copy');
        this.clearButton = document.getElementById('mini-json-clear');
    }
    init() {
    ...
    handleConvert() {
        ...
        try {
            const parsedUnknown = JSON.parse(rawText);
            ...
            const message = error instanceof Error ? error.message : 'JSONの解析中に想定外のエラーが発生しました。';
    ...
    compressJson(value) {
        ...
        const reserved = new Set(keyCounts.keys());
    ...
    generateAlias(original, used, reserved) {
    ...
    async copyLegend() {
        ...
        const lines = this.latestLegendEntries.map(entry => `${entry.alias}: ${entry.original} (${entry.count}回)`);
        try {
    ...
    setLegendCopyVisibility(visible) {
...
exports.MiniJsonComponent = MiniJsonComponent;
